...
<script>
    export default {
        ...
        data() {
            return {
                swiperArr: new Array(5),
                activeIndex: 0
            }
        },
        mounted() {
            this.el = document.querySelector('.swiper-container')        // 容器
            this.itemEls = document.querySelectorAll('.swiper-item')     // 元素
            this.width = this.itemEls[0].offsetWidth                     // 元素宽度，每次滑动的位移
            ...
            touchend() {
                if (this.moveX > 0) {
                    if (this.moveX < this.moveFlag) {
                        ...
                        let step = Math.ceil(this.left / this.width)
                        step++
